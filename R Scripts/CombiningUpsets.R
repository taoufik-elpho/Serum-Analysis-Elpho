...
for(i in 1:length(listsamples)){
  ...
  proteinscount=rbind(proteinscount,data.frame(Sample=str_split(listsamples[i],pattern = "_")[[1]][2],COMET=espcounts[1],MSGF=espcounts[2],XTANDEDM=espcounts[3],MYRIMATCH=espcounts[4],COMBINED=espcounts[5]))
}
